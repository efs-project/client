...
export class EASx extends eassdk.EAS {
   ...
   */
  public async getReferencingAttestationUIDCount(
    attestationUID: string,
    schemaUID: string,
  ): Promise<bigint> {
    try {
      const result = (await publicClient.readContract({
        ...
        args: [attestationUID as `0x${string}`, schemaUID as `0x${string}`],
      })) as bigint;
  ...
  public async indexAttestation(uid: `0x${string}`): Promise<any> {
    ...
    if (!currentSigner) {
      throw new Error(
        'Signer not connected. Please connect a wallet before trying to write to the blockchain.',
      );
    }
...
export async function getReferencingAttestationUIDs(
  ...
  const { Kernel } = await import('../kernel/kernel');
  return Kernel.EFS.EAS.getReferencingAttestationUIDs(
    attestationUID,
    schemaUID,
    start,
    length,
